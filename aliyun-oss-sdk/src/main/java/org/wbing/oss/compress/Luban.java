...
public class Luban implements Handler.Callback {
    ...
    private File getImageCacheFile(Context context, String suffix) {
        if (TextUtils.isEmpty(mTargetDir)) {
            ...
            mTargetDir = cacheDir.getAbsolutePath();
        }
                    ...
                    public void run() {
                        try {
                            ...
                            File result = Checker.isNeedCompress(mLeastCompressSize, path) ?
                                    new Engine(path, getImageCacheFile(context, Checker.checkSuffix(path)), width, height).compress() :
                                    new File(path);
    ...
    private File get(String path, Context context) throws IOException {
        return new Engine(path, getImageCacheFile(context, Checker.checkSuffix(path)), width, height).compress();
    }
    ...
    private List<File> get(Context context) throws IOException {
        ...
        while (iterator.hasNext()) {
            ...
            if (Checker.isImage(path)) {
                results.add(new Engine(path, getImageCacheFile(context, Checker.checkSuffix(path)), width, height).compress());
            }
            iterator.remove();
        }
