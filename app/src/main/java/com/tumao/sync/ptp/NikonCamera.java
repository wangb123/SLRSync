...
import com.tumao.sync.ptp.commands.GetDevicePropDescCommand;
import com.tumao.sync.ptp.commands.InitiateCaptureCommand;
import com.tumao.sync.ptp.commands.RetrieveAddedObjectInfoAction;
import com.tumao.sync.ptp.commands.SimpleCommand;
import com.tumao.sync.ptp.commands.nikon.NikonAfDriveCommand;
import com.tumao.sync.ptp.commands.nikon.NikonCloseSessionAction;
import com.tumao.sync.ptp.commands.nikon.NikonEventCheckCommand;
import com.tumao.sync.ptp.commands.nikon.NikonGetLiveViewImageAction;
import com.tumao.sync.ptp.commands.nikon.NikonGetLiveViewImageCommand;
import com.tumao.sync.ptp.commands.nikon.NikonOpenSessionAction;
import com.tumao.sync.ptp.commands.nikon.NikonStartLiveViewAction;
import com.tumao.sync.ptp.commands.nikon.NikonStopLiveViewAction;
...
public class NikonCamera extends PtpCamera {
    ...
    protected void onOperationCodesReceived(Set<Integer> operations) {
        ...
        if (operations.contains(PtpConstants.Operation.NikonGetLiveViewImage) && operations.contains(PtpConstants.Operation.NikonStartLiveView)
                && operations.contains(PtpConstants.Operation.NikonEndLiveView)) {
            liveViewSupported = true;
        }
        if (operations.contains(PtpConstants.Operation.NikonMfDrive)) {
            driveLensSupported = true;
        }
        if (operations.contains(PtpConstants.Operation.NikonChangeAfArea)) {
            liveViewAfAreaSupported = true;
        }
        if (operations.contains(PtpConstants.Operation.NikonAfDrive)) {
    ...
    public void driveLens(int driveDirection, int pulses) {
        queue.add(new SimpleCommand(this, PtpConstants.Operation.NikonMfDrive, driveDirection == DriveLens.Far ? 0x02 : 0x01,
                pulses * 300));
    }
    ...
    public void setLiveViewAfArea(float posx, float posy) {
        if (supportedOperations.contains(PtpConstants.Operation.NikonChangeAfArea)) {
